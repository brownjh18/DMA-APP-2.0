...
const EditPodcast: React.FC = () => {
  ...
  if (!isLoggedIn || !isAdmin) {
    return (<IonPage><IonContent className="ion-padding"><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '16px' }}>
      <IonSpinner name="crescent" color="primary" /><IonText color="medium"><p style={{ fontSize: '14px' }}>Checking permissions...</p></IonText></div></IonContent></IonPage>);
  }

  if (loading) {
    return (<IonPage><IonContent className="ion-padding"><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: '16px' }}>
      <IonSpinner name="crescent" color="primary" /><IonText color="medium"><p style={{ fontSize: '14px' }}>Loading podcast data...</p></IonText></div></IonContent></IonPage>);
  }
  ...
  return (
    <IonPage>
      <IonHeader translucent>
        <IonToolbar style={{ '--border-width': '0px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 8px' }}>
            <IonTitle style={{ color: isDarkMode ? '#fff' : colors.text, fontWeight: '600', fontSize: '18px', letterSpacing: '-0.3px', textAlign: 'center' }}>Edit Podcast</IonTitle>
          </div>
        </IonToolbar>
      </IonHeader>
      ...
      <IonContent fullscreen className="ion-padding" style={{ background: isDarkMode ? 'transparent' : colors.bg }}>
        {/* Hero Section */}
        <div style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%)', borderRadius: '24px', padding: '32px 24px', marginBottom: '24px', position: 'relative', overflow: 'hidden', boxShadow: '0 20px 60px rgba(102,126,234,0.3)' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
            ...
            <h1 style={{ margin: '0 0 8px 0', fontSize: '24px', fontWeight: '700', color: '#fff', letterSpacing: '-0.5px' }}>Edit Podcast</h1>
            <p style={{ margin: '0', color: 'rgba(255,255,255,0.85)', fontSize: '14px', fontWeight: '400' }}>Update podcast information and media files</p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', background: isDarkMode ? 'rgba(255,255,255,0.05)' : colors.tabBg, borderRadius: '16px', padding: '4px' }}>
          {['draft', 'published', 'scheduled'].map((s) => (
            <button key={s} onClick={() => handleInputChange('status', s)} style={{ flex: 1, padding: '12px 16px', borderRadius: '12px', border: 'none',
              ...
              transition: 'all 0.3s ease', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', textTransform: 'capitalize' }}>
              <IonIcon icon={formData.status === s ? checkmarkCircle : informationCircle} style={{ fontSize: '16px' }} /> {s}
            </button>
          ))}
        </div>

        {/* Form Fields */}
        <div style={{ background: isDarkMode ? 'rgba(255,255,255,0.08)' : colors.cardBg, backdropFilter: isDarkMode ? 'blur(20px)' : 'none',
          ...
          {/* Category */}
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: isDarkMode ? 'rgba(255,255,255,0.7)' : colors.textSecondary, marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Category</label>
            <div style={{ position: 'relative' }}>
              <select value={formData.category} onChange={(e) => handleInputChange('category', e.target.value)}
                ...
                <option value="faith">Faith & Belief</option><option value="prayer">Prayer & Worship</option><option value="teaching">Bible Teaching</option>
                <option value="testimony">Testimonies</option><option value="youth">Youth Ministry</option><option value="family">Family & Relationships</option><option value="other">Other</option>
              </select>
          ...
          {/* Duration */}
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: isDarkMode ? 'rgba(255,255,255,0.7)' : colors.textSecondary, marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Duration</label>
            <div style={{ position: 'relative' }}>
          ...
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: isDarkMode ? 'rgba(255,255,255,0.7)' : colors.textSecondary, marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Description</label>
          ...
          {/* Audio File Upload */}
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: isDarkMode ? 'rgba(255,255,255,0.7)' : colors.textSecondary, marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Audio File</label>
            ...
            {!audioFileName ? (
              <div onClick={() => fileInputRef.current?.click()}
                ...
                <p style={{ margin: '0 0 4px 0', color: isDarkMode ? 'rgba(255,255,255,0.7)' : colors.textSecondary, fontSize: '14px', fontWeight: '500' }}>Replace audio file</p>
                <p style={{ margin: '0', color: isDarkMode ? 'rgba(255,255,255,0.4)' : colors.textMuted, fontSize: '12px' }}>Optional • Max 100MB • MP3, WAV, M4A</p>
              </div>
            ) : (
              <div style={{ borderRadius: '16px', overflow: 'hidden', background: isDarkMode ? 'rgba(34,197,94,0.15)' : colors.videoCardBg, border: `2px solid ${isDarkMode ? 'rgba(34,197,94,0.3)' : colors.successBorder}` }}>
                <div style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ margin: '2px 0 0 0', color: isDarkMode ? 'rgba(255,255,255,0.5)' : colors.textMuted, fontSize: '12px' }}>Audio file ready</p>
                  </div>
          ...
          {/* Thumbnail Upload */}
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: isDarkMode ? 'rgba(255,255,255,0.7)' : colors.textSecondary, marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Podcast Thumbnail</label>
            ...
            {(!thumbnailPreview && !formData.thumbnailUrl) ? (
              ...
              <div style={{ borderRadius: '16px', overflow: 'hidden', background: isDarkMode ? 'rgba(255,255,255,0.05)' : colors.inputBg, border: `2px solid ${isDarkMode ? 'rgba(102,126,234,0.3)' : colors.primary}` }}>
                <img src={thumbnailPreview || formData.thumbnailUrl} alt="Thumbnail preview" style={{ width: '100%', height: '200px', objectFit: 'cover', display: 'block' }} />
                <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <IonIcon icon={image} style={{ color: colors.primary, fontSize: '16px' }} />
                    <span style={{ color: isDarkMode ? 'rgba(255,255,255,0.7)' : colors.textSecondary, fontSize: '13px' }}>Thumbnail {thumbnailPreview ? 'selected' : 'exists'}</span>
                  </div>
        ...
        <button onClick={handleSave} disabled={saving}
          style={{ width: '100%', padding: '16px 32px', borderRadius: '16px', border: 'none',
            background: saving ? (isDarkMode ? 'rgba(255,255,255,0.2)' : colors.loadingBg) : 'linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%)',
            color: '#fff', fontSize: '16px', fontWeight: '600', cursor: saving ? 'not-allowed' : 'pointer',
            boxShadow: saving ? 'none' : '0 8px 32px rgba(102,126,234,0.4), 0 2px 8px rgba(102,126,234,0.2)',
            transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)', opacity: saving ? 0.6 : 1,
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', letterSpacing: '0.3px', position: 'relative', overflow: 'hidden' }}
          onMouseEnter={(e) => { if (!saving) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 12px 40px rgba(102,126,234,0.5), 0 4px 12px rgba(102,126,234,0.3)'; } }}
          onMouseLeave={(e) => { if (!saving) { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 8px 32px rgba(102,126,234,0.4), 0 2px 8px rgba(102,126,234,0.2)'; } }}
