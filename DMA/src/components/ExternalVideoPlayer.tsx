  ...
  if (mini) {
    ...
    return (
      <div style={{
        ...
        <iframe
          key={`mini-external-${url}-${Date.now()}`}
          src={isYouTube ? `${getYouTubeEmbedUrl(url)}?autoplay=${playing ? 1 : 0}&modestbranding=1&rel=0&playsinline=1&iv_load_policy=3&mute=1` : url}
          width="100%"
          height="100%"
          ...
          onError={() => {
            console.log('Mini external iframe error, showing fallback');
  ...
  return (
    <div style={containerStyle}>
      <iframe
        ...
        onError={() => {
          console.log('External iframe error, showing fallback');
          setShowFallback(true);
        }}
      />
    </div>
